...
class Storage:
    ...
    def PutFile(
        ...
        file_name                   : String
                                      The name of the file to be used in storage zone
        storage_directory           : String
                                     The directory path in storage zone where file is to be uploaded
        ...
        # to build correct url
        if storage_directory is not None and storage_directory != "":
            if storage_directory[0] == "/":
                storage_directory = storage_directory[1:]
            if storage_directory[-1] == "/":
                storage_directory = storage_directory[:-1]
            url = (
                self.base_url
                + parse.quote(storage_directory)
                + "/"
        ...
        response = requests.put(url, data=local_file.read(), headers=self.headers)
        try:
            response.raise_for_status()
        except HTTPError as http:
            res = {
    ...
    def DeleteFile(self, storage_path=""):
        ...
        try:
            ...
            return {
                "status": "error",
                "HTTP": response.raise_for_status(),  # type: ignore
                "msg": f"HTTP Error occured: {http}",
            }
        except Exception as err:
            return {
                "status": "error",
                "HTTP": response.status_code,  # type: ignore
